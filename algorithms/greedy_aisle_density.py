...
class GreedyAisleDensityHeuristic(Algorithm):
    ...
    def solve(self, instance: ProblemInput) -> dict:
        ...
        for k in range(initial_k, max_k + 1):
            ...
            if not selected_orders or not cleaned_aisles:
                return {'selected_orders': [], 'visited_aisles': [], 'objective': 0.0}

            total_items = sum(sum(inst.orders[o].values()) for o in selected_orders)
            objective = total_items / len(cleaned_aisles)
            return {'selected_orders': selected_orders, 'visited_aisles': cleaned_aisles, 'objective': objective}
