...
class PingPongAlternatingHeuristic(Algorithm):
    ...
    def solve(self, instance: ProblemInput) -> dict:
        ...
        if not best_orders or not best_aisles:
            fallback_orders, fallback_aisles = self._construct_greedy_fallback()
            selected_orders = fallback_orders
            visited_aisles = fallback_aisles
        else:
